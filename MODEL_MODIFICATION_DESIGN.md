...
即使上次退出前 `modelModificationEnabled=true`，下次启动后也只能恢复为：

```text
总闸开启，状态未等待
```

必须由用户手动点击 `等待下次启动`，才开始等待新的 `Minecraft.Windows.exe`。

停止语义：

- 如果当前处于等待新进程阶段，点击 `停止等待` 必须立刻结束等待。
...
- 等待流程只认用户点击 `等待下次启动` 之后出现的新进程。

成功一次后：

- 如果总闸仍开启，并且用户没有停止，则继续等待下一次 Minecraft 启动。
- 如果用户点击停止或关闭总闸，则本轮完成后退出，不再继续等待。
