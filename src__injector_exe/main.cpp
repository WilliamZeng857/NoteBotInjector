...
static void lockWindowToFixedSize(QWindow *window)
{
    if (!window) {
        return;
    }

    const QSize fixedSize(960, 640);
    window->setMinimumSize(fixedSize);
    window->setMaximumSize(fixedSize);
    window->resize(fixedSize);
...
int main(int argc, char *argv[])
{
    ...
    if (auto *rootWindow = qobject_cast<QWindow *>(engine.rootObjects().constFirst())) {
        lockWindowToFixedSize(rootWindow);
    } else {
        qWarning() << "QML root object is not a QWindow; fixed window size not applied";
    }
